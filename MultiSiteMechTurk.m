...
chnl(1)=struct('Number',1,'ChannelName','TRITC', 'Content','Mis12 Cherry');
chnl(2)=struct('Number',1,'ChannelName','FITC', 'Content','GFP tubulin');
...
GenericTsk_TimeLapse=set(GenericTsk_TimeLapse,'channels',chnls,...
                          'exposuretime',repmat(ExposureTimeLapse,[1 length(Z) length(T)]),...
                          'planetime',repmat(T,[length(chnls) length(Z) 1]),...
                          'stagez',repmat(Z,[length(chnls) 1 length(T)]),...
                          'UserData',UserData,...
...
for i=1:length(OldTsk)
    ...
    if ~isempty(xy)
        ...
        TimeLapse=set(GenericTsk_TimeLapse,...
                          'stagex',repmat(xy(1),[length(chnls) length(Z) length(T)]),...
                          'stagey',repmat(xy(2),[length(chnls) length(Z) length(T)]),...
