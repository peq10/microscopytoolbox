...
for i=1:2:length(varargin)
    switch lower(varargin{i})
        ...
        case 'channels'
            if sum(ismember(fieldnames(varargin{i+1}),{   'Number'; 'ChannelName'; 'Content'; 'ChannelType'}))~=4
                error('Wrong fields for Channel struct')
            end
            varargin{i+1}=orderfields(varargin{i+1},{'Number'; 'ChannelName'; 'Content'; 'ChannelType'});
        ...
        case 'imgheight'
            if ~isa(varargin{i+1},'numeric') || legnth(size(varargin{i+1})) > 3
        ...
        case 'planetime'
            if ~isa(varargin{i+1},'numeric') || legnth(size(varargin{i+1})) > 3
                error('PlaneTime must be numeris and <= 3D')
            end
